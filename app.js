...
const NW_POINT_REFRESH_MIN_MS = 30_000;
...
async function fetchJSON(url, opts = {}) {
  try {
    ...
    // Abort is expected during fast TF switching
...
function setMode(isLive){
  ...
  if (!liveMode) {
    ...
    startKlineWS();
    loadCandleSnapshot();
    try { setPriceTf("live"); } catch {}
    safeAsync(() => loadPriceChart(true), "loadPriceChart");
...
async function fetchKlines1mRange(startTime, endTime) {
  ...
  while (cursor < end && out.length < DAY_MINUTES) {
    ...
    const d = await fetchJSON(url, fetchOpts);
...
function updateChartFrom1mKline(k) {
  // Called by WS kline stream (1m). Keeps the chart moving in LIVE (15m) and updates the last candle in 1D.
  if (!liveMode) return;
  if (!(priceTf === "live" || priceTf === "1d")) return;
  if (!chart) return;

  const t = safe(k?.t);           // open time (ms)
  const close = safe(k?.c);       // close price
  if (!t || !Number.isFinite(close) || close <= 0) return;
  ...
  const lastTs = lastLbl ? labelToTs(lastLbl) : 0;

  if (lastTs === t) {
    // update last candle
    data[data.length - 1] = close;
  } else if (!lastTs || t > lastTs) {
    // append new candle
    labels.push(lbl);
    data.push(close);
  } else {
  ...
  // Trim to window
  if (priceTf === "live") {
    const cut = Date.now() - PRICE_LIVE_WINDOW_MS;
    while (labels.length > 2 && labelToTs(labels[0]) < cut) { labels.shift(); data.shift(); }
    while (labels.length > PRICE_LIVE_MAX_POINTS) { labels.shift(); data.shift(); }
  } else {
    // 1D: keep ~24h of 1m candles
    while (labels.length > 1440) { labels.shift(); data.shift(); }
  }

  // Sync color with selected timeframe (first vs last)
  if (data.length >= 2) {
    const first = safe(data[0]);
    const lastV = safe(data[data.length - 1]);
    const sign = (lastV > first) ? "up" : (lastV < first) ? "down" : "flat";
    applyChartColorBySign(sign);
  }

  // Keep caches fresh (throttled)
  try {
    savePriceCache(priceTf, labels, data);
  } catch {}
...
function maybeRecordRewardWithdrawal(newRewards) {
  ...
  if (diff > REWARD_WITHDRAW_THRESHOLD) {
    ...
    saveWdAllLocal();
    rebuildWdView();
    try { updateTotalRewardAccUI(); } catch {}
...
let _totalRewardsAccLast = null;
let _totalRewardsAccUsdLast = null;
...
function updateTotalRewardAccUI(){
  ...
  const usdVal = total * px;
  const usdRound = Math.round(usdVal * 100) / 100;

  // avoid DOM writes every frame
  if (out && _totalRewardsAccLast !== total){
    out.textContent = total.toFixed(6);
    _totalRewardsAccLast = total;
  }
  if (usd && _totalRewardsAccUsdLast !== usdRound){
    usd.textContent = `≈ $${usdRound.toFixed(2)}`;
    _totalRewardsAccUsdLast = usdRound;
  }
}
...
function recordNetWorthPoint(force=false){
  ...
  const now = Date.now();
  const minMs = liveMode ? NW_POINT_MIN_MS : NW_POINT_REFRESH_MIN_MS;
  if (!force && (now - lastNWPointAt) < minMs) return;

  // Use the freshest known price (works in REFRESH too)
  const px = safe(targetPrice) || safe(displayed.price);
  ...
  if (!force && lastT && dt < minMs && dUsd < 0.50) return;
...
// ✅ When LIVE mode is ON, default the price chart to LIVE (15m)
try{ if (liveMode) priceTf = "live"; } catch {}


/* === Price Chart perf (fast switching) === */
const PRICE_FAST_TTL_MS = 30_000; // skip refetch if data was refreshed recently
const PRICE_LIVE_WINDOW_MS = 15 * 60 * 1000; // ✅ LIVE = last 15 minutes
const PRICE_LIVE_MAX_POINTS = 20;            // keep it light
const PRICE_FAST_TTL_LIVE_MS = 6_000;        // refresh LIVE more often

let priceFetchAbort = null;       // AbortController for in-flight fetch
const priceLastFetch = Object.create(null);
let pricePrefetchScheduled = false;

/* === Price Chart cache (instant TF switch) === */
const PRICE_CACHE_VER = 1;
function priceCacheKey(tf){
  const t = String(tf || "1d").toLowerCase();
  return `inj_price_cache_v${PRICE_CACHE_VER}_${t}`;
}
function loadPriceCache(tf){
  try{
    const raw = localStorage.getItem(priceCacheKey(tf));
    if (!raw) return null;
    const obj = JSON.parse(raw);
    if (!obj || obj.v !== PRICE_CACHE_VER) return null;
    ...
    for (let i = 0; i < n; i++){
      const ts = safe(t[i]);
      const v  = safe(d[i]);
      if (ts > 0 && Number.isFinite(v)){
        labels.push(tsLabel(ts));
        data.push(v);
      }
    }
...
function savePriceCache(tf, labels, data){
  try{
    ...
    const obj = {
      v: PRICE_CACHE_VER,
      tf: String(tf || "1d").toLowerCase(),
      savedAt: Date.now(),
      t: t.slice(-MAX),
      d: d.slice(-MAX)
    };
    localStorage.setItem(priceCacheKey(tf), JSON.stringify(obj));
  } catch {}
}
function applyPriceCache(tf){
  const c = loadPriceCache(tf);
  if (!c) return false;
  if (!chart) initChartToday();
...
/* === Price TF prefetch (super fast switching) === */
function sleep(ms){ return new Promise(r => setTimeout(r, ms)); }
function schedulePricePrefetch(){
  if (pricePrefetchScheduled) return;
  pricePrefetchScheduled = true;

  const run = () => {
    // avoid wasting bandwidth in LITE view
    try{ if (document.body?.dataset?.view === "lite") return; } catch {}
    safeAsync(() => prefetchPriceTFs(), "prefetchPriceTFs");
  };

  if ("requestIdleCallback" in window){
    try{ requestIdleCallback(run, { timeout: 1800 }); return; } catch {}
  }
  setTimeout(run, 1800);
}

async function prefetchPriceTFs(){
  if (!hasInternet()) return;
  const tfs = ["1w","1m"]; // keep it light, these are the most common
  for (const tf of tfs){
    if (tf === priceTf) continue;
    // already cached? skip
    if (loadPriceCache(tf)) continue;
    // small delay between calls
    await fetchAndCachePriceTF(tf);
    await sleep(250);
  }
}

async function fetchAndCachePriceTF(tf){
  const now = Date.now();
  let interval = "1m";
  let start = 0;
  let maxTotal = 1440;
  ...
  if (tf === "live"){
    interval = "1m";
    start = now - PRICE_LIVE_WINDOW_MS;
    maxTotal = PRICE_LIVE_MAX_POINTS;
  } else if (tf === "1d"){
    interval = "1m";
    ...
    interval = "1d";
    start = now - 2400*24*60*60*1000;
    maxTotal = 2400;
  }

  const ctl = new AbortController();
  const kl = await fetchKlinesRange("INJUSDT", interval, start, now, Math.min(2400, maxTotal), { signal: ctl.signal });
  if (!kl || !kl.length) return;
  const labels = kl.map(k => tsLabel(safe(k[0])));
  const data = kl.map(k => safe(k[4]));
  try{ savePriceCache(tf, labels, data); }catch{}
}
...
function updatePriceTitle(){
  ...
  if (!el) return;
  const map = { live:"LIVE • 15m Price Chart", "1d":"1D Price Chart", "1w":"1W Price Chart", "1m":"1M Price Chart", "1y":"1Y Price Chart", all:"ALL Price Chart" };
...
function setPriceTf(tf){
  const next = String(tf || "1d").toLowerCase();
  if (!["live","1d","1w","1m","1y","all"].includes(next)) return;
  ...
  // ✅ instant render using cache (if present)
  try { applyPriceCache(priceTf); } catch {}

  // refresh in background, but skip aggressive refetch if data is still fresh
  safeAsync(() => loadPriceChart(false), "loadPriceChart");

  // apply scale on tf change
  setTimeout(() => applyPriceScale(), 0);

  // prefetch a couple of TFs when idle (makes switching feel instant)
  schedulePricePrefetch();
}
...
async function fetchKlinesRange(symbol, interval, startTime, endTime, maxTotal=2400, fetchOpts={}){
  const out = [];
...
async function loadPriceChart(force=false){
  if (!hasInternet()) return;
  const tf = priceTf; // snapshot (avoid stale updates if user switches TF)
  if (!chart) initChartToday();
  if (!chart) return;

  const now = Date.now();
  const last = priceLastFetch[tf] || 0;

  // If recently refreshed, don't refetch aggressively (makes TF switching feel instant)
  const ttl = (tf === "live") ? PRICE_FAST_TTL_LIVE_MS : PRICE_FAST_TTL_MS;
  if (!force && last && (now - last) < ttl){
    // still ensure the current scale is correct
    try{ applyPriceScale(); } catch {}
    try{ chart.update("none"); } catch {}
    return;
  }

  // cancel previous in-flight request (fast switching)
  try{ priceFetchAbort?.abort?.(); } catch {}
  priceFetchAbort = new AbortController();
  const sig = priceFetchAbort.signal;
  ...
  const kl = await fetchKlinesRange("INJUSDT", interval, start, now, Math.min(2400, maxTotal), { signal: sig });
  ...
  // ✅ also keep the main price in sync even in REFRESH mode
  try{
    const lastClose = safe(data[data.length - 1]);
    if (Number.isFinite(lastClose) && lastClose > 0){
      targetPrice = lastClose;
      if (!displayed.price || !Number.isFinite(displayed.price) || displayed.price <= 0) displayed.price = lastClose;
    }
  } catch {}


  // cache so TF switching feels instant
  try{ savePriceCache(tf, labels, data); }catch{}

  // record fetch time (for TTL)
  priceLastFetch[tf] = Date.now();

  // user switched TF during fetch? don't apply stale data to chart
  if (tf !== priceTf) return;
