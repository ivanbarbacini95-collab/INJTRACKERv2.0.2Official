...
window.addEventListener("error", (e) => {
  setStatusError("JS Error: " + (e?.message || e?.error?.message || "unknown"));
  console.error("JS Error:", e?.error || e);
});
window.addEventListener("unhandledrejection", (e) => {
  setStatusError("Promise Error: " + (e?.reason?.message || String(e?.reason || "unknown")));
...
function setAddressDisplay(addr) {
  ...
  document.body.classList.toggle("no-wallet", !addr);
...
function drawNW(force=false){
  ...
  if (netWorthChart && $("nwReadout")) {
    ...
    if (ds.length) {
      const i = ds.length - 1;
      $("nwReadout").textContent = `${labelToTs(lbs[v]) ? fmtHHMM(labelToTs(lbs[v])) : "—"} • $${safe(ds[i]).toFixed(2)}`;
    }
  }
}
...
await cloudPull();
  ...
  if (liveMode) {
    ...
    setStatusError("Boot Error: " + (e?.message || "unknown"));
...
function animate() {
  ...
  requestAnimationFrame(animate);
    ...
    setStatusError("Loop Error: " + (e?.message || "unknown"));
...
/* ================= FINAL_PATCH_V20260207 =================
   - Fix status dot + blinking (online/loading/offline)
   - Search input: never prefill, but wallet stays stored and loads on refresh
   - Add Preferences reorder (mobile-friendly) inside Settings > Preference
   - Apply saved card order to both Expert/Lite
   ========================================================= */

(function(){
  /* ---------- Status dot blink states ---------- */
  function setStatusBlink(state){
    const dot = $("statusDot");
    if (!dot) return;
    dot.classList.remove("blink-ok","blink-loading","blink-offline");
    if (state === "offline") dot.classList.add("blink-offline");
    else if (state === "loading") dot.classList.add("blink-loading");
    else dot.classList.add("blink-ok");
  }

  const __refreshConnUI = refreshConnUI;
  refreshConnUI = function(){
    if (!statusDot || !statusText) return;

    if (!hasInternet()){
      statusText.textContent = "Offline";
      statusDot.style.background = "#ef4444";
      setStatusBlink("offline");
      return;
    }

    // If LIVE, consider "ready" when sockets are OK and (if wallet set) account loaded at least once
    const liveOk = liveMode ? liveReady() : false;
  ...
  // whenever we reach a ready state, force modeLoading off
  const __loadAccount = loadAccount;
  loadAccount = async function(isRefresh=false){
    await __loadAccount(isRefresh);
    if (liveMode && liveReady()) modeLoading = false;
    refreshConnUI();
  };

  const __refreshLoadAllOnce = refreshLoadAllOnce;
  refreshLoadAllOnce = async function(){
    await __refreshLoadAllOnce();
    modeLoading = false;
    refreshConnUI();
  };

  /* ---------- Search input: do NOT prefill ---------- */
  openSearch = function(){
    if (!searchWrap) return;
    searchWrap.classList.add("open");
    document.body.classList.add("search-open");
    if (addressInput){
      addressInput.value = ""; // ✅ never show saved wallet here
      pendingAddress = "";
  ...
  const CARD_TITLES = [
    ...
    { id:"injprice",   label:"INJ Price" },
    { id:"validator",  label:"Validator" },
    { id:"eventsmini", label:"Events" },
  ];
  ...
  function currentOrderFromDOM(){
    ...
    if (!wrap) return CARD_TITLES.map(x=>x.id);
    const nodes = [...wrap.querySelectorAll(".card[data-card]")];
    const ids = nodes.map(n => String(n.dataset.card||"")).filter(Boolean);
    return ids.length ? ids : CARD_TITLES.map(x=>x.id);
  }
  ...
  function applyCardOrder(order){
    ...
    const cards = [...wrap.querySelectorAll(".card[data-card]")];
    const map = new Map(cards.map(c => [String(c.dataset.card), c]));

    const base = CARD_TITLES.map(x=>x.id);
    const ord = Array.isArray(order) ? order.slice() : (getSavedOrder() || base);

    // ensure all present + no unknown
    const seen = new Set();
    const final = [];
    for (const id of ord){
      if (!map.has(id) || seen.has(id)) continue;
      final.push(id); seen.add(id);
    }
    for (const id of base){
      if (!seen.has(id) && map.has(id)) final.push(id);
    }
  ...
  function renderPrefList(){
    ...
    const order = getSavedOrder() || currentOrderFromDOM() || CARD_TITLES.map(x=>x.id);
    const labelMap = new Map(CARD_TITLES.map(x => [x.id, x.label]));
    ...
    const buildRow = (id) => {
      ...
      row.innerHTML = `
        <div class="pref-left">
          <div class="pref-handle" title="Drag / move">☰</div>
          <div class="pref-title">${labelMap.get(id) || id}</div>
        </div>
        <div class="pref-actions">
          <button class="pref-move" data-move="up" aria-label="Move up">↑</button>
          <button class="pref-move" data-move="down" aria-label="Move down">↓</button>
        </div>
      `;
      return row;
    };

    const items = order.map(id => buildRow(id));
    items.forEach(el => host.appendChild(el));

    // Up/down buttons (mobile friendly)
    host.addEventListener("click", (e) => {
      const btn = e.target?.closest?.(".pref-move");
      if (!btn) return;
      const row = btn.closest(".pref-item");
      if (!row) return;

      const dir = btn.dataset.move;
      const list = [...host.querySelectorAll(".pref-item")];
      const idx = list.indexOf(row);
      if (idx < 0) return;

      if (dir === "up" && idx > 0){
        host.insertBefore(row, list[idx-1]);
      } else if (dir === "down" && idx < list.length - 1){
        host.insertBefore(list[idx+1], row);
      }
    }, { passive:true });
    ...
    host.addEventListener("dragstart", (e) => {
      dragEl = e.target?.closest?.(".pref-item") || null;
      if (dragEl){
        e.dataTransfer.effectAllowed = "move";
        try{ e.dataTransfer.setData("text/plain", dragEl.dataset.cardId || ""); } catch {}
      }
    });
    host.addEventListener("dragover", (e) => {
      if (!dragEl) return;
      e.preventDefault();
      const over = e.target?.closest?.(".pref-item");
      if (!over || over === dragEl) return;
      const rect = over.getBoundingClientRect();
      const after = (e.clientY - rect.top) > rect.height/2;
      host.insertBefore(dragEl, after ? over.nextSibling : over);
    });
    host.addEventListener("drop", (e) => { e.preventDefault(); dragEl = null; });
    host.addEventListener("dragend", () => { dragEl = null; });

    // Touch reorder (simple): long-press drag handle to move with finger
    let touchRow = null;
    let touchStartY = 0;
    let touchActive = false;

    host.addEventListener("touchstart", (e) => {
      const h = e.target?.closest?.(".pref-handle");
      if (!h) return;
      const r = h.closest(".pref-item");
      if (!r) return;
      touchRow = r;
    ...
    host.addEventListener("touchmove", (e) => {
      if (!touchActive || !touchRow) return;
      ...
      const list = [...host.querySelectorAll(".pref-item")];
      const idx = list.indexOf(touchRow);
      if (dy < 0 && idx > 0){
        host.insertBefore(touchRow, list[idx-1]);
      } else if (dy > 0 && idx < list.length - 1){
        host.insertBefore(list[idx+1], touchRow);
      }
    }, { passive:true });
  ...
  function readOrderFromPref(){
    const host = $("prefList");
    if (!host) return null;
    return [...host.querySelectorAll(".pref-item")].map(x => String(x.dataset.cardId||"")).filter(Boolean);
  }

  function bindPreferenceAccordion(){
    const btn = $("prefAccBtn");
    const body = $("prefAccBody");
    if (!btn || !body) return;
    btn.addEventListener("click", () => {
      const open = btn.getAttribute("aria-expanded") === "true";
      btn.setAttribute("aria-expanded", open ? "false" : "true");
      body.setAttribute("aria-hidden", open ? "true" : "false");
      if (!open) renderPrefList();
    }, { passive:true });

    $("prefApply")?.addEventListener("click", (e) => {
      e.preventDefault();
      const ord = readOrderFromPref();
      if (ord && ord.length){
        applyCardOrder(ord);
        pushEvent({ kind:"info", title:"Preferences saved", detail:"Dashboard order updated.", status:"done" });
      }
    }, { passive:false });

    $("prefReset")?.addEventListener("click", (e) => {
      e.preventDefault();
      const base = CARD_TITLES.map(x=>x.id);
      saveOrder(base);
      applyCardOrder(base);
      renderPrefList();
      pushEvent({ kind:"info", title:"Preferences reset", detail:"Default order restored.", status:"done" });
    }, { passive:false });
  }

  // apply saved order ASAP
  applyCardOrder(getSavedOrder());

  // bind preference accordion once DOM exists
  bindPreferenceAccordion();
