...
const MIN_GAS_PRICE_INJ = 160_000_000;
const TX_GAS_WANTED_EST = 110_000;
...
function setExtremeBlink(el, val, low, high){
  if (!el) return;
  el.classList.remove("blink-ath","blink-atl");
  val = safe(val); low = safe(low); high = safe(high);
  if (!Number.isFinite(val) || !Number.isFinite(low) || !Number.isFinite(high) || high === low) return;

  // tolerance to avoid flicker due to floating rounding / feed jitter
  const eps = Math.max(1e-12, Math.abs(high - low) * 0.0002); // 0.02% of range
  if (val >= high - eps) el.classList.add("blink-ath");
  else if (val <= low + eps) el.classList.add("blink-atl");
}
...
function startAllTimers(){
  ...
  chartSyncTimer = setInterval(() => {
    ...
    // In LIVE: WS keeps LIVE/1D updated; refresh only if WS is offline or TF is not 1m-based
    if (!(priceTf === "live" || priceTf === "1d") || !wsKlineOnline) {
...
function ensureChartBootstrapped(){
  ...
  const ok = Array.isArray(lbs) && Array.isArray(ds) && lbs.length > 2 && ds.length > 2;

  if (!ok){
    // bootstrap from Binance regardless of mode/address
    return loadPriceChart(true);
  }

  // keep axis scale aligned with current toggle
...
let priceChartInterval = "1m";
let priceChartWindowMs = 24 * 60 * 60 * 1000;
let priceChartMaxPoints = DAY_MINUTES;
...
function updateChartFrom1mKline(k) {
  ...
  const ds = chart?.data?.datasets?.[0]?.data || [];
  if (!Array.isArray(labels) || !Array.isArray(ds) || !labels.length || !ds.length) return;

  // accept only points within the current window (avoid old klines)
  const now = Date.now();
  const windowMs = (priceTf === "live") ? PRICE_LIVE_WINDOW_MS : (24*60*60*1000);
  if (openTime < (now - windowMs - ONE_MIN_MS)) return;

  const lastTs = labelToTs(labels[labels.length - 1]) || 0;

  // same minute: update last point
  if (openTime === lastChartMinuteStart || (lastTs && Math.abs(openTime - lastTs) < 500)) {
    ds[ds.length - 1] = close;
  } else if (openTime > lastTs) {
    labels.push(tsLabel(openTime));
    ds.push(close);
  } else {
    return; // out-of-order
  }
  ...
  const maxPts = (priceTf === "live") ? PRICE_LIVE_MAX_POINTS : DAY_MINUTES;
  while (labels.length > maxPts) labels.shift();
  while (ds.length > maxPts) ds.shift();

  // color by TF performance
  if (priceTf === "live") applyChartColorBySign("live");
  else {
    const first = safe(ds[0]);
    const last = safe(ds[ds.length - 1]);
    applyChartColorBySign((last > first) ? "up" : (last < first) ? "down" : "flat");
  }
...
function rebuildWdView() {
  ...
  try { if (typeof updateValidatorFeeUI === "function") updateValidatorFeeUI(); } catch (e) { console.warn("[updateValidatorFeeUI]", e); }
}
...
function updateTotalRewardAccUI(){
  const card = $("totalRewardsAccCard");
...
const lastDotPlugin = {
  ...
  afterDatasetsDraw(ch){
    ...
    const isLivePrice = (ch?.canvas?.id === "priceChart" && priceTf === "live");
...
function setPriceTf(tf){
  ...
  // load chart for the selected TF, then re-apply axis scale
  safeAsync(async () => {
    await loadPriceChart(true);
    try{ applyPriceScale(); } catch {}
  }, "loadPriceChart(tf)");
...
async function loadPriceChart(force=false){
  ...
  if (priceTf === "live"){
    ...
    maxTotal = Math.max(PRICE_LIVE_MAX_POINTS + 3, 20);
  } else if (priceTf === "1d"){
    interval = "1m";
  ...
  // remember current chart sampling/window (used by WS updates)
  priceChartInterval = interval;
  priceChartWindowMs =
    (priceTf === "live") ? PRICE_LIVE_WINDOW_MS :
    (priceTf === "1d") ? (24*60*60*1000) :
    (priceTf === "1w") ? (7*24*60*60*1000) :
    (priceTf === "1m") ? (30*24*60*60*1000) :
    (priceTf === "1y") ? (365*24*60*60*1000) :
    0;
  priceChartMaxPoints =
    (priceTf === "live") ? PRICE_LIVE_MAX_POINTS :
    (priceTf === "1d") ? DAY_MINUTES :
    maxTotal;
  ...
  const clippedLabels = (priceTf === "live") ? labels.slice(-PRICE_LIVE_MAX_POINTS) : labels;
  const clippedData   = (priceTf === "live") ? data.slice(-PRICE_LIVE_MAX_POINTS)   : data;

  chart.data.labels = clippedLabels;
  chart.data.datasets[0].data = clippedData;
  ...
  const first = safe(clippedData[0]);
  const last = safe(clippedData[clippedData.length - 1]);
  const sign = (priceTf === "live") ? "live" : (last > first) ? "up" : (last < first) ? "down" : "flat";
  applyChartColorBySign(sign);

  try{ applyPriceScale(); } catch {}

  chartBootstrappedToday = (interval === "1m");
  lastChartMinuteStart = safe(kl?.[kl.length - 1]?.[0]);
...
/* === Validator: fee needed for a tx (est.) === */
let lastValidatorFeeUiAt = 0;
function estimateTxFeeInj(gasWanted = TX_GAS_WANTED_EST, minGasPriceInj = MIN_GAS_PRICE_INJ){
  // minGasPriceInj is in base denom "inj" (1 INJ = 1e18 inj)
  return (safe(minGasPriceInj) * safe(gasWanted)) / 1e18;
}
function updateValidatorFeeUI(){
  const el = $("validatorFeeLine");
  if (!el) return;

  const now = Date.now();
  if ((now - lastValidatorFeeUiAt) < 450) return;
  lastValidatorFeeUiAt = now;

  const feeInj = estimateTxFeeInj();
  const px = (Number.isFinite(displayed?.price) && displayed.price > 0) ? displayed.price :
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  // Price chart loads independently (faster on first paint)
  updatePriceTFButtons();
  updatePriceTitle();
  safeAsync(() => loadPriceChart(true), "boot loadPriceChart");
...
function animate() {
  ...
  if (tfReady.d) {
    ...
    setExtremeBlink($("priceOpen"), targetPrice, candle.d.low, candle.d.high);
  } else { setText("priceMin", "--"); setText("priceOpen", "--"); setText("priceMax", "--"); }
  try{ $("priceOpen")?.classList.remove("blink-ath","blink-atl"); }catch{}
  ...
  if (tfReady.w) {
    ...
    setExtremeBlink($("weekOpen"), targetPrice, candle.w.low, candle.w.high);
  } else { setText("weekMin", "--"); setText("weekOpen", "--"); setText("weekMax", "--"); }
  try{ $("weekOpen")?.classList.remove("blink-ath","blink-atl"); }catch{}
  ...
  if (tfReady.m) {
    ...
    setExtremeBlink($("monthOpen"), targetPrice, candle.m.low, candle.m.high);
  } else { setText("monthMin", "--"); setText("monthOpen", "--"); setText("monthMax", "--"); }
  try{ $("monthOpen")?.classList.remove("blink-ath","blink-atl"); }catch{}
  ...
  if (tfReady.y) {
    ...
    setExtremeBlink($("yearOpen"), targetPrice, candle.y.low, candle.y.high);
  } else { setText("yearMin", "--"); setText("yearOpen", "--"); setText("yearMax", "--"); }
  try{ $("yearOpen")?.classList.remove("blink-ath","blink-atl"); }catch{}
