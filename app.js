...
async function loadAccount(isRefresh=false) {
  ...
  try{
    ...
    // Fetch independently so one endpoint doesn't block everything
    const bankP = fetchJSON(`${base}/cosmos/bank/v1beta1/balances/${address}`);
    const delP  = fetchJSON(`${base}/cosmos/staking/v1beta1/delegations/${address}`);
    const rewP  = fetchJSON(`${base}/cosmos/distribution/v1beta1/delegators/${address}/rewards`);
    const infP  = fetchJSON(`${base}/cosmos/mint/v1beta1/inflation`);
