...
function initStakeChart() {
  ...
  stakeChart = new Chart(canvas, {
    ...
    options: {
      ...
      interaction: { mode: "index", intersect: false },
      scales: {
        x: { ticks: { color: axisTickColor(), maxRotation: 0, autoSkip: true, maxTicksLimit: 6, padding: 6,
          callback: (v) => {
...
<script>
(() => {
  const ENDPOINT = "/api/track"; // stesso dominio
  const deviceKey = "trk_device_id_v1";
  ...
  post({ type: "start", sessionId, deviceId, deviceInfo, page: location.pathname, ts: now() });
  ...
  function startBeats() {
    ...
    beatTimer = setInterval(() => {
      if (document.visibilityState === "visible") {
        post({ type: "beat", sessionId, deviceId, ts: now() });
      }
    }, 10000); // ogni 10s
  }
  function stopBeats() {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") startBeats();
    else stopBeats();
  });

  window.addEventListener("pagehide", () => post({ type: "end", sessionId, deviceId, ts: now(), reason: "pagehide" }));
  window.addEventListener("beforeunload", () => post({ type: "end", sessionId, deviceId, ts: now(), reason: "unload" }));
